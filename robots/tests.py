...
from .utils import get_info_robots, get_dataframe
...
from robots.utils import get_info_robots, get_dataframe
...
class ExportToExcelTests(TestCase):
    ...
    def test_response_metadata(self):
        ...
        self.assertIn(
            'attachment; filename=robots_by_models.xlsx',
    ...
    def test_excel_structure(self):
        ...
        r2_sheet = wb['R2']
        self.assertEqual(
            [cell.value for cell in r2_sheet[1]],
        ...
        self.assertEqual(
            [cell.value for cell in data_rows[0]],
            ['R2', 'D2', 2]
        )
...
class GetInfoRobotsTests(TestCase):
    def setUp(self):
        ...
        now = timezone.now()
        Robot.objects.create(
            model='R2', version='D2', created=now - timedelta(days=6))
        Robot.objects.create(
            model='R2', version='D2', created=now - timedelta(days=8))  # Старая запись
...
class GetDataframeTests(TestCase):
    def test_dataframe_creation(self):
        ...
        test_data = [
            RobotToExcel(model='R2', version='D2', total=5),
            RobotToExcel(model='C3', version='PO', total=3)
        ]
